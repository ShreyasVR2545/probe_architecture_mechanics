...
\hline
$0.10$ & $0.00\,/\,0.00$ & $\mathbf{1.00\,/\,1.00}$ & $0.92\,/\,1.00$\\
...
Measured failure boundary (lowest $m$ with recall $<0.80$, $S=2.0$, $N=16{,}384$):
MultiMax $m=64$, softmax pooling $m=256$, mean pooling $m=1$. Softmax pooling tolerates
a $4\times$ wider spread than MultiMax before failing, while mean pooling fails
immediately for the reason \eqref{eq:mean-snr} gives.
\end{remark}
...
the weak-signal regime ($1.00$ vs $0.92$ at $s=0.10$) and ties elsewhere. Claims of
superiority should be stated as conditional on $k/N$ and the achievable logit gap.

\emph{(iii) Against a distributed adversary, MultiMax is strictly worse}
(Remark~\ref{rem:distributed}): it fails at $m=64$ where softmax pooling survives to
$m=256$.
