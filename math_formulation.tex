...
\label{rem:scope}
Combining Prop.~\ref{prop:mean}, Prop.~\ref{prop:softmax} and
Remark~\ref{rem:gradient-sparsity}: the demonstrated advantage of the hard maximum is
over \emph{mean} aggregation, whose $\Theta(1/N)$ decay has no free parameter to absorb
it. It is \emph{not} established over single-query softmax pooling, because $\gamma$ in
\eqref{eq:softmax-mass} is a trained quantity and a well-separated needle drives it to
$e^{\gamma}\!\sim\!N/k$, cancelling the dilution exactly. Remark~\ref{rem:gamma} argues
$\gamma$ cannot grow \emph{without bound}; the measurements show that at $N=16{,}384$ it
does not need to. Claims of MultiMax superiority over attention pooling should be stated
as conditional on the dilution ratio $k/N$ and the achievable logit gap, not as
categorical.
\end{remark}
...
self-attention \emph{across} the sequence. MultiMax's advantage over single-query
pooling is therefore statistical (Theorem~\ref{thm:multimax}), while its advantage in
memory is the constant-in-$N$ streaming reduction, not an asymptotic separation.
\end{remark}
