...
\author{%
  \normalsize Shreyas Venkata Ramanan\\[1pt]
  % NOTE: the affiliation supplied read "Shreyas Venkata RamananTechnology Bombay",
  % which is a truncated concatenation. The name is used as given; the institution is
  % left as an explicit placeholder rather than guessed, since an affiliation is a
  % factual claim about a person. Replace the line below before submission.
  \normalsize \textit{[affiliation to be confirmed]}\\[1pt]
...
$\alpha_j=\mathrm{softmax}_j(q^\top y_j/\sqrt m)$ the corresponding support is
\begin{equation}
  \frac{\partial}{\partial y_j}\Big(w^\top\!\sum_{j'}\alpha_{j'}y_{j'}\Big)
  \;\ne\;0 \quad\text{for every } j,
  \qquad\text{so}\qquad
...
of at most $H$ indices. For softmax, $\alpha_j>0$ strictly for all $j$ because the
exponential is positive, and the derivative of the pooled vector with respect to $y_j$
contains the term $\alpha_j w$, which is non-zero; no cancellation occurs for generic $w$.
\end{proof}
...
dictionaries do not reliably beat random dictionaries of matched shape.
...
never re-reads. At $N{=}131{,}072$, $m{=}512$ in bf16 that is roughly $128$\,MiB of avoidable
round-trip per sequence, and it is the reason the latency gap ($12.729$ vs $9.205$\,ms)
is smaller than the memory gap ($652.1$ vs $19.1$\,MiB): the kernels are bandwidth-bound,
not arithmetic-bound, and bandwidth is what the streaming reduction buys.
...
regime where the maximum fails. This is the mechanism behind the error-decorrelation that
\citet{koran2026ensemble} observe empirically, and it suggests aggregator diversity is the
principled axis on which to build a monitor ensemble --- at the cost of reintroducing a
$\Theta(N)$ member unless that member is itself streamed.
